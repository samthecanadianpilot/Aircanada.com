...
import { FaLink, FaPlane, FaPlaneDeparture } from 'react-icons/fa';
...
export default function BookFlight() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      const found = liveFlights.find((activeFlight) => activeFlight.id === flightId);
      const existingTaken = JSON.parse(localStorage.getItem(`taken_${flightId}`) || '[]');
  ...
  if (!flight) {
    return (
      <div className="container section text-center">
        ...
        <button className="btn btn-outline" style={{ marginTop: '24px' }} onClick={() => router.push('/')}>
          Return Home
        </button>
      </div>
    );
  ...
  const handleBook = () => {
    const booking = {
      ...
      passengerName: name,
      seat: selectedSeat,
      refNumber: `AC-${Math.random().toString(36).slice(2, 8).toUpperCase()}`,
    ...
    <div className="container section">
      <div className="booking-header text-center animate-in">
        ...
        <p style={{ fontSize: '1.2rem', color: 'var(--text)', fontWeight: 500 }}>
          {flight.origin} &nbsp; <FaPlaneDeparture color="var(--ac-red)" /> &nbsp; {flight.destination}
        </p>
        <div style={{ marginTop: '16px', display: 'flex', justifyContent: 'center', gap: '16px', color: 'var(--text-secondary)' }}>
          <span>
            <FaPlane /> {flight.aircraft}
          </span>
          <span>
            <FaLink /> Event Link Attached
          </span>
        </div>
      </div>
        ...
        {step === 1 && (
          <>
            ...
            <div className="seat-legend">
              ...
              <span>
                <span className="legend-dot staff" /> Staff Only
              </span>
            </div>

            <div className="plane-fuselage glass-panel" style={{ maxWidth: '340px', margin: '0 auto', padding: '32px 24px' }}>
              <div style={{ textAlign: 'center', marginBottom: '16px', fontSize: '0.8rem', color: '#42A5F5', fontWeight: 700, letterSpacing: '1px' }}>
                STAFF (ROWS 1-2)
              </div>
              <div className="seat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '32px' }}>
                {Array.from({ length: 2 }).map((_, rowIndex) =>
                  Array.from({ length: seatConfig.cols }).map((_, columnIndex) => {
                    const label = getSeatLabel(rowIndex, columnIndex);
                    ...
                    return (
                      <div
                        key={label}
                        className={`seat staff-zone ${isTaken ? 'taken' : ''} ${isSelected ? 'selected' : ''}`}
                        onClick={() => !isTaken && setSelectedSeat(label)}
                      >
                        {label}
                      </div>
                    );
              ...
              <div style={{ textAlign: 'center', marginBottom: '16px', fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 700, letterSpacing: '1px' }}>
                PASSENGERS (ROWS 3-5)
              </div>
              <div className="seat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
                {Array.from({ length: 3 }).map((_, rowIndex) =>
                  Array.from({ length: seatConfig.cols }).map((_, columnIndex) => {
                    const label = getSeatLabel(rowIndex + 2, columnIndex);
                    const isTaken = takenSeats.includes(label);
                    const isSelected = selectedSeat === label;
        ...
        {step === 2 && (
          <div className="glass-panel" style={{ maxWidth: '400px', margin: '0 auto', padding: '32px' }}>
            ...
            <div className="form-group">
              <label>
                Roblox Username <span style={{ color: 'var(--ac-red)' }}>*</span>
              </label>
              <input
                type="text"
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '32px' }}>
              ...
              <button className="btn btn-red" style={{ flex: 1 }} disabled={!name.trim()} onClick={handleBook}>
                Join Flight
              </button>
            </div>
          </div>
